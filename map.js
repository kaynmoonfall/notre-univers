    ...
    const mapData = [
        { 
            ...
            radius: 150, 
            img: "img/1.jpg", 
            msg: "Le début de notre histoire au Parc Coquibus... ❤️" 
        },
        { 
            ...
            radius: 150, 
            img: "img/2.jpg", 
            msg: "Ce bracelet qui ne te quitte plus... ❤️" 
        },
        { 
        ...
        { 
            ...
            radius: 150, 
            img: "img/5.jpg", 
            msg: "Même ici, on a nos souvenirs... 🏠🦠" 
        },
        { 
    ...
    function updateCirclesRadius() {
        ...
        if (zoom <= 10) dynamicRadius = 2000;
        else if (zoom === 11) dynamicRadius = 1200;
        else if (zoom === 12) dynamicRadius = 800;
        else if (zoom === 13) dynamicRadius = 500;
        else if (zoom === 14) dynamicRadius = 300;
        else dynamicRadius = 150;
    ...
    function refreshBlueZones() {
        mapData.forEach((zone, index) => {
            let circle = L.circle([zone.lat, zone.lng], {
                stroke: false, // Pas de bordure bleue
                fillColor: '#3b82f6',
                fillOpacity: 0.4,
                radius: zone.radius,
                interactive: true 
    ...
    function nextMapQuestion() {
        ...
        // On transforme la zone bleue validée en zone noire "Historique"
        L.circle([data.lat, data.lng], {
            stroke: false,      // Pas de bordure
            fillColor: '#000000',
            fillOpacity: 0.4,    // Même opacité que le bleu (0.4)
            radius: data.radius,
            interactive: false   // Pour ne pas gêner les clics sur les cercles bleus dessous
        }).addTo(leafletMap);

        closeMapModal();
        currentMapIndex++;
        updateMapQuestion();
        updateCirclesRadius(); // Pour que le nouveau cercle noir s'adapte aussi au zoom
    }
